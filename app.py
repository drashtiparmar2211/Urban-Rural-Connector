...
class Order(db.Model):
    ...
    pickup_id = db.Column(db.String(20))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    # --- NEW ADDRESS COLUMNS ---
    pickup_address = db.Column(db.String(500))   # Where the driver goes first (Farm)
    delivery_address = db.Column(db.String(500)) # Where the driver goes last (Buyer)
    def __repr__(self):
...
def my_orders():
    ...
    # 1. Fetch orders where user_id matches the person logged in
    # We use .order_by to show the newest orders at the top
    user_orders = Order.query.filter_by(user_id=session['user_id']).order_by(Order.id.desc()).all()
    
    # --- DEBUG SECTION ---
    # Look at your terminal (black window) after you refresh the page
    print(f"DEBUG: Logged in User ID is {current_user}")
    print(f"DEBUG: Found {len(user_orders)} orders in the database for this user.")
    # ---------------------

    # 2. IMPORTANT: The name on the left (orders) MUST match 
    # the name in your html file ({% for order in orders %})
...
def process_payment():
    ...
    d_address = request.form.get('delivery_address')
    d_type = request.form.get('delivery_type', 'standard')
    
    # FIX: Get product IDs from the SESSION cart, not the database table [cite: 350]
    cart_ids = session.get('cart', [])
    if not cart_ids:
        flash("Your cart is empty!", "warning")
        return redirect(url_for('shop'))
    ...
    try:
        ...
        for item in cart_items:
            # Assign Driver [cite: 480-481]
            drv = db.session.execute(text('SELECT id FROM drivers WHERE is_available=1 ORDER BY RANDOM() LIMIT 1')).fetchone()
            driver_id = drv[0] if drv else None

            delivery_fee = 50.0 if d_type == 'instant' else 20.0
            final_total = item.price + delivery_fee

            # Create Order [cite: 486-497]
            new_order = Order(
                user_id=session['user_id'],
                seller_id=item.seller_id, # This works now because 'item' is a Product object
                product_name=item.title,
                price=item.price,
                total_price=final_total, 
                delivery_type=d_type,
                driver_id=driver_id,
                pickup_address="B/81 Radha Krishna Society, Opp Sahyog Park, Gorwa, Vadodara",
            ...
            db.session.refresh(new_order) # Refresh ID [cite: 500]
            order_ids.append(str(new_order.id))

            # OTPs [cite: 503-514]
            if d_type != 'self':
                for o_type in ["pickup", "delivery"]:
                    otp = "".join(random.choices(string.digits, k=6))
                    db.session.execute(text('''
                        INSERT INTO verification_otps (order_id, otp_type, otp_code, expires_at)
                        VALUES (:oid, :otype, :ocode, :exp)
                    '''), {"oid": new_order.id, "otype": o_type, "ocode": otp, "exp": "2026-12-31 23:59:59"})
                db.session.commit()

        # SUCCESS: Clear the session cart [cite: 356]
        session.pop('cart', None)
        
        ids_str = ", ".join(order_ids)
        flash(f"Order placed successfully! Your Order ID(s): #{ids_str}", "success")
        return redirect(url_for('my_orders'))
